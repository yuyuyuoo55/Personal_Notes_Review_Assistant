...
实际 ZIP 导入截图显示：本次返回 29 个片段并成功识别 10 张图片，但知识库列表一度统计为 58 个片段，正好重复一倍。

本次补充处理：

- 选择 ZIP 后提前提示图片需要逐张识别，可能耗时数分钟。
- 点击导入后显示持续加载动画、当前处理说明和已等待时间。
- 文本片段和图片片段均使用稳定 `chunk_id` 作为 Chroma 记录 ID。
- 图片片段 ID 不再包含每次随机生成的本地保存路径，改为由文档、图片序号、原始引用和描述生成。
- Streamlit 网络重连或重复提交时，相同 ID 会覆盖更新，不再生成重复向量记录。
- 新增稳定 ID 测试，最终全量回归结果更新为 `28 passed`。

已经产生的 58 条旧记录不会自动删除。部署新版本后，应先在知识库删除该笔记，再重新上传 ZIP，列表应恢复为本次实际导入的片段数量。
